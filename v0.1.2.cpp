...
void pazymiuIvedimas (std::string kaipIvestiPazymius, int i, int sk, int ndSkaicius, Studentas rezultatai[])
            ...
            while ( kiek <=0 || std::cin.fail())
             ...
             {      std:: cout << "Namu darbu pazymiai: ";
                 for(int k=0; k<kiek; k++)
                {
                 ...
                 std:: cout  << input << " ";
                 rezultatai[i].namuDarbai.push_back(input);
                }
         ...
         while (rezultatai[i].egzaminas <= 0 || rezultatai[i].egzaminas > 10 || std::cin.fail())
            {
                ...
                pazymiuIvedimas (kaipIvestiPazymius, i, sk, ndSkaicius, rezultatai);
        }
    ...
    for (int i = 0; i<kiekStudentu; i++)
        {  double nd = 0;
           ...
           for (int z=0; z<=ndSkaicius; z++){
            ...
            if (z+1==ndSkaicius){rezultatai[i].namuDarbai.push_back(rezultatai[i].egzaminas);}
           }
    ...
    if (arZinoKiek == "Taip" || arZinoKiek == "taip")
        {   std::cout << "Iveskite namu darbu skaiciu: ";
            ...
            while (ndSkaicius <= 0 || std::cin.fail()) {
                    std::cin.clear();
                    std::cin.ignore(std::numeric_limits<std::streamsize>::max(),'\n');
                    std::cout << "Ivestas netinkamas skaicius, bandykite is naujo: ";
                     std::cin >> ndSkaicius; }
            if (ndSkaicius >= 1)
