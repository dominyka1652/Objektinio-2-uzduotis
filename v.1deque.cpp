...
{   int kiek1 = 0, kiek2 = 0;
    ...
    if (vidurkisArMediana == "vidurki") std::sort(rezultatai.begin(), rezultatai.end(), SortByVid);
        ...
        for (int i=0; i<kiekStudentu; i++){
            ...
            if (rezultatai[i].galutinisVid < 5)
                kiek2++;
        }
        else if (vidurkisArMediana == "mediana"){
           if (rezultatai[i].galutinisMed < 5)
                 kiek2++;
        }
            if((vidurkisArMediana == "vidurki" && rezultatai[i].galutinisVid >= 5) ||(vidurkisArMediana == "mediana" && rezultatai[i].galutinisMed >=5))
                break;
        }
        for (int j=0;j<kiek2;j++)
        tinginiai.push_back(rezultatai[j]);
        rezultatai.erase(rezultatai.begin(),rezultatai.begin()+kiek2);
