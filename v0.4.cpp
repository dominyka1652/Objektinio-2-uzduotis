...
{   int kiek1 = 0, kiek2 = 0;
    ...
    tinginiai.reserve(kiekStudentu);
        ...
        std::cout << std::to_string(kiek) + " elementu isspausdinimas i saunuolius uztruko: "<< diff.count() << " s\n";
        start = std::chrono::high_resolution_clock::now();
        Spausdinimas(vidurkisArMediana,tinginiai,kiek2,tinginukai);
        end = std::chrono::high_resolution_clock::now(); // Stabdyti
        diff = end-start; // Skirtumas (s)
        std::cout << std::to_string(kiek) + " elementu isspausdinimas i tinginius uztruko: "<< diff.count() << " s\n";
        std::cout << std::endl;
...
void vidurkisMediana (std::vector<Studentas> &rezultatai, int kiekStudentu, std::string sukurtiFailus, std::string vidurkisArMediana){
   ...
   if (sukurtiFailus== "nenoriu") {std::cout << "Norite gauti aritmetini vidurki (irasykite 'vidurki')\n ar mediana (irasykite 'mediana')?: ";
    ...
    if (vidurkisArMediana == "vidurki" || vidurkisArMediana == "mediana"){
        ...
        if(sukurtiFailus == "sukurti") SkirtytiStudentus(vidurkisArMediana,rezultatai,kiekStudentu);
        else {std::string failiukas = "Rezultatai.txt";
        Spausdinimas(vidurkisArMediana,rezultatai,kiekStudentu, failiukas);
        rezultatai = {};
        }
    }
...
catch(std::exception& e)
{
    ...
    getline(fd, line);
        ...
        {       while(! fd.eof()){
            ...
            fd.close();
   if (sukurtiFailus == "nenoriu") vidurkisMediana(rezultatai,kiekStudentu,sukurtiFailus, vidurkisArMediana);
}
...
void print (int& n, std::vector<Studentas> &rezultatai, int& ndSk, std::string pav){
    ...
    for (int i = 0; i<n; i++){
    gen << std::left << std::setw(20) << rezultatai[i].vardas;
    gen <<  std::left << std::setw(21) << rezultatai[i].pavarde;
        for (int j = 0; j<ndSk; j++)
    gen <<  std::left << std::setw(7) << rezultatai[i].namuDarbai[j];
    gen <<  std::left << std::setw(10) << rezultatai[i].egzaminas << "\n";}
    gen.close();
    rezultatai = {};
}
